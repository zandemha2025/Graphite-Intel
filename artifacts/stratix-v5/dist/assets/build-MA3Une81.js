import{u as J,r as d,j as e,i as B}from"./vendor-DGf4ukVm.js";import{b as Z}from"./vendor-query-CH6xEl-b.js";import{o as ee,p as te,q as ne}from"./pages-boards-zJSX3SxU.js";import{u as ae}from"./pages-notebooks-DB58Rjsl.js";import{O as k,ao as re,ap as oe,X as ie,aq as x,b as L,ar as C,as as E,at as se,au as ce,av as de,aa as le,aw as pe,w as me,ax as ue,ay as ye,az as he,aA as ge,aB as ve,aC as fe,aD as be,W as we,aE as ke,aF as xe,aG as Ce,aH as _e,aI as Se,J as Ae,a3 as Re,r as Pe,aJ as Me,ai as Te,V as Ne,y as Ie,k as je,N as Fe,aK as Be,E as Le,aL as Ee,aM as Oe,i as ze,j as $e,ab as De,l as We,G as Ve,aN as Ke,aO as qe,al as Ge,e as O}from"./vendor-icons-Dfv43Rrw.js";import"./vendor-ui-DNU0vQe8.js";const He=[{key:"all",label:"All"},{key:"strategy",label:"Strategy"},{key:"intelligence",label:"Intelligence"},{key:"marketing",label:"Marketing"},{key:"finance",label:"Finance"},{key:"research",label:"Research"}],_=[{id:"strategy-competitive-analysis",title:"Competitive Analysis",description:"Deep-dive into competitor positioning, strengths, and gaps",category:"strategy",icon:"Sparkles",cells:[{type:"markdown",content:`# Competitive Analysis
...
1. 
2. 
3. `},{type:"ai-prompt",content:"Based on the literature review on [topic], identify the 3 biggest gaps in current research that represent opportunities for [company] to generate proprietary insights. What data or experiments would be needed to fill these gaps?"}]}];function Ue(i){return i==="all"?_:_.filter(m=>m.category===i)}function Ye(i,m="all"){const l=Ue(m);if(!i)return l;const a=i.toLowerCase();return l.filter(v=>v.title.toLowerCase().includes(a)||v.description.toLowerCase().includes(a))}const Qe={Sparkles:O,FileBarChart:Ge,CalendarDays:qe,Megaphone:Ke,Globe:Ve,Target:We,Shield:De,TrendingUp:$e,BarChart3:ze,Link:Oe,Rocket:Ee,Eye:Le,MessageCircle:Be,Newspaper:Fe,PieChart:je,Heart:Ie,DollarSign:Ne,GitBranch:Te,Palette:Me,Filter:Pe,Users:Re,Mail:Ae,Calculator:Se,ClipboardList:_e,Coins:Ce,Flame:xe,FileSpreadsheet:ke,SlidersHorizontal:we,Scale:be,MessageSquare:fe,UserCircle:ve,Maximize:ge,Cpu:he,GitCompare:ye,ClipboardCheck:ue,Radar:me,FileKey:pe,BookOpen:le,LayoutGrid:C,Waves:de,Crosshair:ce,Sunrise:se};function Xe(i){return Qe[i]||O}const Je={strategy:"bg-indigo-100 text-indigo-700",intelligence:"bg-amber-100 text-amber-700",marketing:"bg-emerald-100 text-emerald-700",finance:"bg-rose-100 text-rose-700",research:"bg-sky-100 text-sky-700"};function it(){const[,i]=J(),m=Z(),{toast:l}=ae(),[a,v]=d.useState("all"),[u,z]=d.useState(""),[S,$]=d.useState("edited"),[f,A]=d.useState([]),[b,R]=d.useState("grid"),[p,D]=d.useState(()=>{try{return new Set(JSON.parse(localStorage.getItem("stratix:favorites")||"[]"))}catch{return new Set}}),[P,W]=d.useState("all"),[M,V]=d.useState(""),[y,h]=d.useState(null),T=Ye(M,P),{data:N=[],isError:K,refetch:q}=ee(),G=te();d.useEffect(()=>{fetch("/api/notebooks",{credentials:"include"}).then(t=>t.ok?t.json():[]).then(t=>A(Array.isArray(t)?t:[])).catch(()=>A([]))},[]),d.useEffect(()=>{try{localStorage.setItem("stratix:favorites",JSON.stringify([...p]))}catch{}},[p]);const H=Array.isArray(N)?N:[],U=Array.isArray(f)?f:[],g=[...a==="all"||a==="notebooks"||a==="favorites"?U.map(t=>({...t,kind:"notebook",href:`/build/notebooks/${t.id}`})):[],...a==="all"||a==="boards"||a==="favorites"?H.map(t=>({...t,kind:"board",cellCount:0,href:`/build/boards/${t.id}`})):[]].filter(t=>{const n=p.has(`${t.kind}-${t.id}`);return a==="favorites"?n&&(!u||t.title.toLowerCase().includes(u.toLowerCase())):!u||t.title.toLowerCase().includes(u.toLowerCase())}).sort((t,n)=>S==="name"?t.title.localeCompare(n.title):new Date(n.updatedAt).getTime()-new Date(t.updatedAt).getTime()),I=(t,n)=>{const r=`${t}-${n}`,o=new Set(p);o.has(r)?o.delete(r):o.add(r),D(o)},Y=async()=>{try{const t="Untitled Notebook",n=new Set(f.map(s=>s.title));let r=t;if(n.has(t)){let s=2;for(;n.has(`${t} ${s}`);)s++;r=`${t} ${s}`}const o=await fetch("/api/notebooks",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({title:r,description:""})});if(o.ok){const s=await o.json();i(`/build/notebooks/${s.id}`)}}catch{l({title:"Failed to create notebook",variant:"destructive"})}},Q=()=>{G.mutate({data:{title:"Untitled Board",type:"live"}},{onSuccess:t=>{m.invalidateQueries({queryKey:ne()}),i(`/build/boards/${t.id}`)},onError:()=>{l({title:"Failed to create board",variant:"destructive"})}})},j=async t=>{try{const n=t.title,r=new Set(f.map(c=>c.title));let o=n;if(r.has(n)){let c=2;for(;r.has(`${n} ${c}`);)c++;o=`${n} ${c}`}const s=await fetch("/api/notebooks",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({title:o,description:t.description})});if(!s.ok){l({title:"Failed to create notebook",variant:"destructive"});return}const F=await s.json();for(let c=0;c<t.cells.length;c++){const w=t.cells[c],X=w.type==="markdown"?`Section ${c+1}`:w.type==="ai-prompt"?`AI Analysis ${c+1}`:`Code ${c+1}`;await fetch(`/api/notebooks/${F.id}/cells`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({prompt:w.content,title:X,cellIndex:c})})}l({title:"Template applied",description:`Created "${o}" with ${t.cells.length} cells`}),i(`/build/notebooks/${F.id}`)}catch{l({title:"Failed to apply template",variant:"destructive"})}};return e.jsxs("div",{children:[e.jsx("h1",{className:"font-editorial text-[28px] font-medium text-[var(--text-primary)]",children:"Build"}),e.jsxs("div",{className:"flex items-center justify-between mt-6 gap-4",children:[e.jsxs("div",{className:"relative flex-1 max-w-sm",children:[e.jsx(k,{className:"absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--text-muted)]"}),e.jsx("input",{value:u,onChange:t=>z(t.target.value),placeholder:"Search...",className:"w-full pl-10 pr-4 h-9 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] text-body-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/20"})]}),e.jsxs("div",{className:"flex items-center gap-4",children:[e.jsx("div",{className:"flex items-center gap-1",children:["all","notebooks","boards","favorites"].map(t=>e.jsx("button",{onClick:()=>v(t),className:`px-3 py-1.5 text-body-sm rounded-[var(--radius-sm)] transition-colors ${a===t?"text-[var(--text-primary)] font-medium bg-[var(--surface)]":"text-[var(--text-muted)] hover:text-[var(--text-secondary)]"}`,children:t==="favorites"?"Favorites":t.charAt(0).toUpperCase()+t.slice(1)},t))}),e.jsxs("select",{value:S,onChange:t=>$(t.target.value),className:"h-9 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3 text-body-sm text-[var(--text-secondary)]",children:[e.jsx("option",{value:"edited",children:"Last edited"}),e.jsx("option",{value:"name",children:"Name A-Z"})]}),e.jsxs("div",{className:"flex items-center gap-1 border-l border-[var(--border)] pl-4",children:[e.jsx("button",{onClick:()=>R("grid"),className:`p-2 rounded-[var(--radius-sm)] transition-colors ${b==="grid"?"bg-[var(--surface)] text-[var(--text-primary)]":"text-[var(--text-muted)] hover:text-[var(--text-secondary)]"}`,children:e.jsx(re,{className:"h-4 w-4"})}),e.jsx("button",{onClick:()=>R("list"),className:`p-2 rounded-[var(--radius-sm)] transition-colors ${b==="list"?"bg-[var(--surface)] text-[var(--text-primary)]":"text-[var(--text-muted)] hover:text-[var(--text-secondary)]"}`,children:e.jsx(oe,{className:"h-4 w-4"})})]})]})]}),K&&e.jsxs("div",{className:"flex items-center justify-between mt-6 px-4 py-3 rounded-[var(--radius-md)] bg-[var(--error-muted)] border border-[var(--error)]/20",children:[e.jsx("p",{className:"text-body-sm text-[var(--error)]",children:"Failed to load boards. The server returned an error."}),e.jsx("button",{onClick:()=>q(),className:"ml-4 shrink-0 px-3 py-1.5 text-body-sm font-medium rounded-[var(--radius-sm)] bg-[var(--accent)] text-white hover:opacity-90 transition-opacity",children:"Retry"})]}),(a==="all"||a==="notebooks")&&e.jsxs("div",{className:"mt-8 mb-8",children:[e.jsx("div",{className:"flex items-center justify-between mb-4",children:e.jsxs("div",{className:"flex items-center gap-3",children:[e.jsx("h2",{className:"text-[18px] font-medium text-[var(--text-primary)]",children:"Start from Template"}),e.jsxs("span",{className:"px-2.5 py-0.5 text-[11px] font-medium rounded-full bg-[var(--accent)]/10 text-[var(--accent)]",children:[_.length,"+ templates"]})]})}),e.jsx("div",{className:"flex items-center gap-2 mb-4",children:He.map(t=>e.jsx("button",{onClick:()=>W(t.key),className:`px-3 py-1.5 text-body-sm rounded-[var(--radius-sm)] transition-colors ${P===t.key?"text-[var(--text-primary)] font-medium bg-[var(--surface)] border border-[var(--border-strong)]":"text-[var(--text-muted)] hover:text-[var(--text-secondary)] border border-transparent"}`,children:t.label},t.key))}),e.jsxs("div",{className:"relative max-w-xs mb-4",children:[e.jsx(k,{className:"absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--text-muted)]"}),e.jsx("input",{value:M,onChange:t=>V(t.target.value),placeholder:"Filter templates...",className:"w-full pl-10 pr-4 h-9 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] text-body-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/20"})]}),e.jsx("div",{className:"grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4",children:T.map(t=>{const n=Xe(t.icon);return e.jsxs("div",{className:"flex flex-col rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] hover:bg-[var(--surface-elevated)] transition-all overflow-hidden group",children:[e.jsxs("div",{className:"flex-1 flex items-center justify-center bg-[var(--surface-secondary)] py-6 relative",children:[e.jsx(n,{className:"h-8 w-8 text-[var(--accent)]"}),e.jsx("button",{onClick:()=>h(t),className:"absolute top-2 right-2 px-2 py-1 text-[10px] font-medium rounded-[var(--radius-sm)] bg-white/80 hover:bg-white text-[var(--text-secondary)] opacity-0 group-hover:opacity-100 transition-all",children:"Preview"})]}),e.jsxs("div",{className:"px-3 py-3",children:[e.jsx("div",{className:"flex items-center gap-2 mb-1",children:e.jsx("p",{className:"text-body-sm font-medium text-[var(--text-primary)] truncate",children:t.title})}),e.jsx("p",{className:"text-[11px] text-[var(--text-muted)] mt-0.5 line-clamp-2",children:t.description}),e.jsxs("div",{className:"flex items-center justify-between mt-2",children:[e.jsx("span",{className:`px-2 py-0.5 text-[10px] font-medium rounded-full ${Je[t.category]}`,children:t.category}),e.jsxs("span",{className:"text-[10px] text-[var(--text-muted)]",children:[t.cells.length," cells"]})]}),e.jsx("button",{onClick:()=>j(t),className:"mt-3 w-full px-3 py-1.5 text-[11px] font-medium rounded-[var(--radius-sm)] bg-[var(--accent)] text-white hover:opacity-90 transition-opacity",children:"Use Template"})]})]},t.id)})}),T.length===0&&e.jsxs("div",{className:"flex flex-col items-center py-8 text-center",children:[e.jsx(k,{className:"h-8 w-8 text-[var(--text-muted)] mb-2"}),e.jsx("p",{className:"text-body-sm text-[var(--text-muted)]",children:"No templates match your search."})]})]}),y&&e.jsx("div",{className:"fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm",onClick:()=>h(null),children:e.jsxs("div",{className:"relative w-full max-w-2xl max-h-[80vh] overflow-y-auto rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg)] shadow-2xl mx-4",onClick:t=>t.stopPropagation(),children:[e.jsxs("div",{className:"sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-[var(--border)] bg-[var(--bg)]",children:[e.jsxs("div",{children:[e.jsx("h3",{className:"text-[18px] font-medium text-[var(--text-primary)]",children:y.title}),e.jsx("p",{className:"text-body-sm text-[var(--text-muted)] mt-0.5",children:y.description})]}),e.jsx("button",{onClick:()=>h(null),className:"p-2 rounded-[var(--radius-sm)] hover:bg-[var(--surface-secondary)] transition-colors",children:e.jsx(ie,{className:"h-5 w-5 text-[var(--text-muted)]"})})]}),e.jsx("div",{className:"px-6 py-4 space-y-3",children:y.cells.map((t,n)=>e.jsxs("div",{className:"rounded-[var(--radius-md)] border border-[var(--border)] overflow-hidden",children:[e.jsxs("div",{className:"flex items-center gap-2 px-3 py-1.5 bg-[var(--surface-secondary)] border-b border-[var(--border)]",children:[e.jsx("span",{className:`px-2 py-0.5 text-[10px] font-medium rounded-full ${t.type==="markdown"?"bg-blue-100 text-blue-700":t.type==="ai-prompt"?"bg-purple-100 text-purple-700":"bg-green-100 text-green-700"}`,children:t.type==="ai-prompt"?"AI Prompt":t.type==="markdown"?"Markdown":"Code"}),e.jsxs("span",{className:"text-[10px] text-[var(--text-muted)]",children:["Cell ",n+1]})]}),e.jsx("pre",{className:"px-3 py-2.5 text-[12px] text-[var(--text-secondary)] whitespace-pre-wrap font-mono leading-relaxed max-h-40 overflow-y-auto bg-[var(--surface)]",children:t.content})]},n))}),e.jsxs("div",{className:"sticky bottom-0 flex items-center justify-end gap-3 px-6 py-4 border-t border-[var(--border)] bg-[var(--bg)]",children:[e.jsx("button",{onClick:()=>h(null),className:"px-4 py-2 text-body-sm rounded-[var(--radius-sm)] border border-[var(--border)] text-[var(--text-secondary)] hover:bg-[var(--surface-secondary)] transition-colors",children:"Close"}),e.jsx("button",{onClick:()=>{j(y),h(null)},className:"px-4 py-2 text-body-sm font-medium rounded-[var(--radius-sm)] bg-[var(--accent)] text-white hover:opacity-90 transition-opacity",children:"Use Template"})]})]})}),g.length===0&&e.jsxs("div",{className:"flex flex-col items-center justify-center mt-12 py-12",children:[e.jsx("div",{className:"w-24 h-24 rounded-full bg-[var(--surface-secondary)] flex items-center justify-center mb-4",children:e.jsx(x,{className:"h-12 w-12 text-[var(--text-muted)]"})}),e.jsx("p",{className:"text-body-md text-[var(--text-secondary)] mt-2",children:"Your workspace is empty."}),e.jsx("p",{className:"text-body-sm text-[var(--text-muted)] mt-1",children:"Create a notebook or board, or start from a template."})]}),g.length>0&&b==="grid"&&e.jsxs("div",{className:"grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-6",children:[(a==="all"||a==="notebooks")&&e.jsxs("button",{onClick:Y,className:"flex flex-col items-center justify-center h-40 rounded-[var(--radius-lg)] border-2 border-dashed border-[var(--border)] hover:border-[var(--border-strong)] transition-colors",children:[e.jsx(L,{className:"h-6 w-6 text-[var(--text-muted)] mb-2"}),e.jsx("span",{className:"text-body-sm text-[var(--text-primary)]",children:"New Notebook"})]}),(a==="all"||a==="boards")&&e.jsxs("button",{onClick:Q,className:"flex flex-col items-center justify-center h-40 rounded-[var(--radius-lg)] border-2 border-dashed border-[var(--border)] hover:border-[var(--border-strong)] transition-colors",children:[e.jsx(L,{className:"h-6 w-6 text-[var(--text-muted)] mb-2"}),e.jsx("span",{className:"text-body-sm text-[var(--text-primary)]",children:"New Board"})]}),g.map(t=>{const n=t.kind==="notebook"?x:C,r=`${t.kind}-${t.id}`,o=p.has(r);return e.jsxs("div",{className:"flex flex-col h-40 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] hover:bg-[var(--surface-elevated)] hover:shadow-md transition-all overflow-hidden group",children:[e.jsxs("div",{className:"flex-1 flex items-center justify-center bg-[var(--surface-secondary)] relative cursor-pointer",onClick:()=>i(t.href),children:[e.jsx(n,{className:"h-8 w-8 text-[var(--border-strong)]"}),e.jsx("button",{onClick:s=>{s.stopPropagation(),I(t.kind,t.id)},className:"absolute top-2 right-2 p-1.5 rounded-[var(--radius-sm)] bg-white/80 hover:bg-white transition-all opacity-0 group-hover:opacity-100",children:e.jsx(E,{className:`h-4 w-4 ${o?"fill-[var(--accent)] text-[var(--accent)]":"text-[var(--text-muted)]"}`})})]}),e.jsxs("div",{className:"px-3 py-2.5 cursor-pointer",onClick:()=>i(t.href),children:[e.jsx("p",{className:"text-body-sm font-medium text-[var(--text-primary)] truncate",children:t.title}),e.jsx("p",{className:"text-[11px] text-[var(--text-muted)] mt-0.5",children:B(new Date(t.updatedAt),"MMM d, yyyy")})]})]},r)})]}),g.length>0&&b==="list"&&e.jsx("div",{className:"space-y-2 mt-6",children:g.map(t=>{const n=t.kind==="notebook"?x:C,r=`${t.kind}-${t.id}`,o=p.has(r);return e.jsxs("div",{onClick:()=>i(t.href),className:"flex items-center gap-4 px-4 py-3 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] hover:bg-[var(--surface-elevated)] transition-all cursor-pointer group",children:[e.jsx(n,{className:"h-5 w-5 text-[var(--border-strong)] shrink-0"}),e.jsxs("div",{className:"flex-1",children:[e.jsx("p",{className:"text-body-sm font-medium text-[var(--text-primary)]",children:t.title}),e.jsx("p",{className:"text-[11px] text-[var(--text-muted)]",children:B(new Date(t.updatedAt),"MMM d, yyyy")})]}),e.jsx("button",{onClick:s=>{s.stopPropagation(),I(t.kind,t.id)},className:"p-1.5 rounded-[var(--radius-sm)] hover:bg-[var(--surface-secondary)] transition-all opacity-0 group-hover:opacity-100",children:e.jsx(E,{className:`h-4 w-4 ${o?"fill-[var(--accent)] text-[var(--accent)]":"text-[var(--text-muted)]"}`})})]},r)})})]})}export{it as Build};
