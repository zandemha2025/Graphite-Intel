  ...
  if (phase === "url-entry") {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--background)] px-4">
        <form onSubmit={(e) => { e.preventDefault(); if (companyUrl.trim()) { setPhase("researching"); } }} className="w-full max-w-lg text-center">
          ...
          <div className="relative mb-6">
            ...
            <input autoFocus type="text" value={companyUrl} onChange={(e) => setCompanyUrl(e.target.value)} placeholder="acmecorp.com" className="w-full pl-12 pr-4 h-14 rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface)] text-xl text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/20" />
          </div>
      ...
      try {
        ...
        while (true) {
          ...
          for (const part of parts) {
            ...
            if (!data) continue;
            try { const p = JSON.parse(data); if (event === "status") setLines((prev) => prev.includes(p.message) ? prev : [...prev, p.message]); else if (event === "complete") { clearTimeout(timeout); onComplete(p); } else if (event === "error") { clearTimeout(timeout); onError(p.error); } } catch {}
          }
        }
      } catch { onError("Connection error."); } finally { clearTimeout(timeout); }
    })();
