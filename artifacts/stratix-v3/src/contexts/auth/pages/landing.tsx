    ...
    <div className="min-h-screen bg-[#F8F8F7]">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-[#E5E5E3]/40">
        <div className="flex items-center justify-between px-6 py-3.5 max-w-6xl mx-auto">
          <div className="flex items-center gap-2.5 text-lg font-semibold text-[#1A1A1A]">
            <div className="w-8 h-8 rounded-xl bg-[#1A1A1A] flex items-center justify-center text-white text-xs font-bold">S</div>
            Stratix
          </div>
          <nav className="hidden md:flex items-center gap-8 text-sm text-[#525252]">
            ...
            <a href="#why" className="hover:text-[#1A1A1A] transition-colors">Why Stratix</a>
            <a href="#security" className="hover:text-[#1A1A1A] transition-colors">Security</a>
          </nav>
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => navigate("/login")} className="text-[#525252] hover:text-[#1A1A1A]">
              Sign in
            </Button>
      ...
      {/* Hero */}
      <motion.section className="text-center px-6 pt-24 pb-20 max-w-3xl mx-auto" initial="hidden" animate="visible" variants={stagger}>
        ...
        <motion.h1 className="text-5xl md:text-[56px] font-semibold tracking-[-0.025em] text-[#1A1A1A] leading-[1.1] max-w-2xl mx-auto" variants={fadeUp}>
        ...
        <motion.form className="mt-10 max-w-md mx-auto" onSubmit={handleGetStarted} variants={fadeUp}>
          <div className="flex items-center bg-white border border-[#E5E5E3] rounded-xl shadow-sm hover:shadow-md transition-shadow p-1.5">
            ...
            />
            <Button type="submit" size="md" className="whitespace-nowrap h-10 px-5 shrink-0">
              Get started free
              <ArrowRight className="w-3.5 h-3.5 ml-1" />
            </Button>
          </div>
          <p className="text-xs text-[#A3A3A3] mt-3">No credit card required. Free for teams up to 5.</p>
        </motion.form>
      </motion.section>

      {/* Product Preview */}
      <motion.section className="max-w-4xl mx-auto px-6 pb-24" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={fadeUp}>
            ...
            {previewTabs.map((tab, i) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-3.5 text-sm transition-all relative ${
                  i === activeTab ? "text-[#1A1A1A] font-medium" : "text-[#A3A3A3] hover:text-[#525252]"
                }`}
              >
                <tab.icon className="w-4 h-4" />
                {tab.label}
                {i === activeTab && <span className="absolute bottom-0 left-2 right-2 h-[2px] bg-[#1A1A1A] rounded-full" />}
              </button>
            ))}
          </div>
            ...
            {activeTab === 0 ? (
              <div className="space-y-3 max-w-xl mx-auto pt-4">
                <div className="flex items-center gap-3 bg-white rounded-xl border border-[#E5E5E3] px-4 py-3 shadow-sm">
                  ...
                  <span className="text-sm text-[#A3A3A3]">Ask anything about your market...</span>
                </div>
                {mockConversations.map((conv) => (
                  <div key={conv.title} className={`bg-gradient-to-r ${conv.gradient} rounded-xl px-4 py-3.5 flex items-center justify-between group cursor-default`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-[#1A1A1A]">{conv.title}</p>
                        <p className="text-xs text-[#A3A3A3]">{conv.time}</p>
                      </div>
                    </div>
                    <ArrowRight className="w-4 h-4 text-[#A3A3A3] opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                ))}
              </div>
            ) : (
                  ...
                  return (
                    <div className="text-center">
                      <div className="w-14 h-14 rounded-2xl bg-white shadow-sm border border-[#E5E5E3]/50 flex items-center justify-center mx-auto mb-4">
                        <TabIcon className="w-6 h-6 text-[#A3A3A3]" />
                      </div>
                      <p className="text-sm text-[#A3A3A3] font-medium">{current.label}</p>
      ...
      {/* Features */}
      <motion.section id="features" className="max-w-5xl mx-auto px-6 pb-28" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger}>
        <motion.h2 className="text-3xl font-semibold tracking-tight text-[#1A1A1A] text-center mb-3" variants={fadeUp}>
          ...
          {features.map((f) => (
            <motion.div key={f.title} variants={fadeUp}>
              <div className="bg-white rounded-2xl p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] transition-all duration-200 h-full border border-[#E5E5E3]/30">
                ...
                <h3 className="text-base font-medium text-[#1A1A1A] mb-1.5">{f.title}</h3>
      ...
      {/* Why Stratix */}
      <motion.section id="why" className="max-w-4xl mx-auto px-6 pb-28" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger}>
        <motion.h2 className="text-3xl font-semibold tracking-tight text-[#1A1A1A] text-center mb-3" variants={fadeUp}>
          Why Stratix
        </motion.h2>
        ...
        <motion.div variants={fadeUp} className="hidden md:grid grid-cols-[1fr_1fr_1fr] gap-4 mb-3 px-5">
          <span className="text-xs font-medium text-[#A3A3A3] uppercase tracking-wider">Scenario</span>
          <span className="text-xs font-medium text-[#A3A3A3] uppercase tracking-wider">General AI</span>
          <span className="text-xs font-medium text-[#1A1A1A] uppercase tracking-wider flex items-center gap-1.5">
            <div className="w-4 h-4 rounded bg-[#1A1A1A] flex items-center justify-center text-white text-[8px] font-bold">S</div>
            Stratix
          </span>
        </motion.div>
              ...
              <div className="bg-white rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] border border-[#E5E5E3]/30 overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr] gap-0">
                  <div className="p-5 md:border-r border-b md:border-b-0 border-[#E5E5E3]/50">
                    <p className="text-xs font-medium text-[#A3A3A3] uppercase tracking-wide mb-1.5 md:hidden">Scenario</p>
                    <p className="text-sm font-medium text-[#1A1A1A]">{c.scenario}</p>
                  </div>
                  <div className="p-5 md:border-r border-b md:border-b-0 border-[#E5E5E3]/50">
                    <p className="text-xs font-medium text-[#A3A3A3] uppercase tracking-wide mb-1.5 md:hidden">General AI</p>
                    <div className="flex items-start gap-2">
                      <X className="w-4 h-4 text-[#D4D4D4] mt-0.5 shrink-0" />
                      <p className="text-sm text-[#A3A3A3]">{c.general}</p>
                    </div>
                  </div>
                  <div className="p-5 bg-[#FAFFF8]">
                    <p className="text-xs font-medium text-[#1A1A1A] uppercase tracking-wide mb-1.5 md:hidden">Stratix</p>
                    <div className="flex items-start gap-2">
                      <Check className="w-4 h-4 text-[#22C55E] mt-0.5 shrink-0" />
                      <p className="text-sm text-[#1A1A1A] font-medium">{c.stratix}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Integrations */}
      <motion.section className="max-w-3xl mx-auto px-6 pb-28 text-center" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger}>
        <motion.h2 className="text-3xl font-semibold tracking-tight text-[#1A1A1A] mb-3" variants={fadeUp}>600+ integrations</motion.h2>
        <motion.p className="text-base text-[#525252] mb-10" variants={fadeUp}>Connect to the tools and data sources your team already uses.</motion.p>
        <motion.div className="flex flex-wrap justify-center gap-2.5" variants={fadeUp}>
          {integrationCategories.map((cat) => (
            <span key={cat} className="inline-flex items-center rounded-full border border-[#E5E5E3] bg-white px-3 py-1.5 text-xs font-medium text-[#525252] shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
              {cat}
            </span>
          ))}
        </motion.div>
      ...
      {/* Trust */}
      <motion.section id="security" className="max-w-3xl mx-auto px-6 pb-28 text-center" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger}>
        <motion.h2 className="text-3xl font-semibold tracking-tight text-[#1A1A1A] mb-3" variants={fadeUp}>Enterprise-grade security</motion.h2>
        <motion.p className="text-base text-[#525252] mb-10" variants={fadeUp}>Your data is protected with industry-leading security standards.</motion.p>
            ...
            <motion.div key={b.label} variants={fadeUp}>
              <div className="bg-white rounded-2xl p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)] border border-[#E5E5E3]/30 flex flex-col items-center gap-3 hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] transition-all duration-200 group">
                <div className="w-11 h-11 rounded-xl bg-[#F5F5F4] flex items-center justify-center group-hover:bg-[#F0EFED] transition-colors">
                  <b.icon className="w-5 h-5 text-[#1A1A1A]" />
                </div>
      ...
      {/* CTA */}
      <motion.section className="max-w-3xl mx-auto px-6 pb-28 text-center" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={fadeUp}>
        <div className="bg-[#1A1A1A] rounded-3xl p-12 text-white">
          <h2 className="text-3xl font-semibold tracking-tight mb-3">Ready to get started?</h2>
          <p className="text-[#A3A3A3] mb-8 max-w-md mx-auto">Join teams that use Stratix to make better strategic decisions, faster.</p>
          <Button onClick={() => navigate("/login")} className="bg-white text-[#1A1A1A] hover:bg-[#F5F5F4] h-11 px-6 text-sm font-medium">
            Start for free
      ...
      {/* Footer */}
      <footer className="border-t border-[#E5E5E3]/60 bg-white">
        <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-[#A3A3A3]">
          <div className="flex items-center gap-2 text-[#1A1A1A] font-semibold">
            <div className="w-6 h-6 rounded-lg bg-[#1A1A1A] flex items-center justify-center text-white text-[10px] font-bold">S</div>
            Stratix
          </div>
